...
export const listStages = asyncHandler(async (req: Request, res: Response) => {
  const stages = await stageService.listStages(req.orgId!, req.params.id, req.user!.id, req.permissions!);
  res.json(new ApiResponse(200, stages, "Stages"));
});

export const createStage = asyncHandler(async (req: Request, res: Response) => {
  const stage = await stageService.createStage(req.orgId!, req.params.id, req.user!.id, req.permissions!, req.body);
...
export const updateStage = asyncHandler(async (req: Request, res: Response) => {
  const stage = await stageService.updateStage(req.orgId!, req.params.id, req.user!.id, req.permissions!, req.body);
...
export const reorderStages = asyncHandler(async (req: Request, res: Response) => {
  await stageService.reorderStages(req.orgId!, req.params.id, req.user!.id, req.permissions!, req.body.orderedIds);
