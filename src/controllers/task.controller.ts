...
export const listTasks = asyncHandler(async (req: Request, res: Response) => {
  const tasks = await taskService.listTasks(req.orgId!, req.params.id, req.user!.id, req.permissions!, req.query);
  res.json(new ApiResponse(200, tasks, "Tasks"));
});

export const listMyTasks = asyncHandler(async (req: Request, res: Response) => {
  const tasks = await taskService.listMyTasks(req.orgId!, req.user!.id);
  res.json(new ApiResponse(200, tasks, "My tasks"));
});

export const createTask = asyncHandler(async (req: Request, res: Response) => {
  const task = await taskService.createTask(req.orgId!, req.params.id, req.user!.id, req.permissions!, req.body);
  emitToProject(req.params.id, "pm:task:created", { task });
  res.status(201).json(new ApiResponse(201, task, "Task created"));
});

export const getTask = asyncHandler(async (req: Request, res: Response) => {
  const task = await taskService.getTaskForAccess(req.orgId!, req.params.id, req.user!.id, req.permissions!);
  res.json(new ApiResponse(200, task, "Task"));
});

export const updateTask = asyncHandler(async (req: Request, res: Response) => {
  const task = await taskService.updateTask(req.orgId!, req.params.id, req.user!.id, req.permissions!, req.body);
  emitToProject(task.projectId.toString(), "pm:task:updated", { task });
  res.json(new ApiResponse(200, task, "Task updated"));
});

export const deleteTask = asyncHandler(async (req: Request, res: Response) => {
  const task = await taskService.deleteTask(req.orgId!, req.params.id, req.user!.id, req.permissions!);
...
export const moveTask = asyncHandler(async (req: Request, res: Response) => {
  const task = await taskService.moveTask(req.orgId!, req.params.id, req.user!.id, req.permissions!, req.body);
...
export const createSubtask = asyncHandler(async (req: Request, res: Response) => {
  const task = await taskService.createSubtask(req.orgId!, req.params.id, req.user!.id, req.permissions!, req.body);
...
export const listSubtasks = asyncHandler(async (req: Request, res: Response) => {
  const subtasks = await taskService.listSubtasks(req.orgId!, req.params.id, req.user!.id, req.permissions!);
  res.json(new ApiResponse(200, subtasks, "Subtasks"));
});

export const addChecklistItem = asyncHandler(async (req: Request, res: Response) => {
  const task = await taskService.addChecklistItem(req.orgId!, req.params.id, req.user!.id, req.permissions!, req.body.text);
...
export const addAttachment = asyncHandler(async (req: Request, res: Response) => {
  ...
  for (const file of files) {
    ...
    task = await taskService.addAttachment(req.orgId!, req.params.id, req.user!.id, req.permissions!, {
      ...stored,
