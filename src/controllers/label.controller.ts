...
export const listLabels = asyncHandler(async (req: Request, res: Response) => {
  const labels = await labelService.listLabels(req.orgId!, req.params.id, req.user!.id, req.permissions!);
  res.json(new ApiResponse(200, labels, "Labels"));
});

export const createLabel = asyncHandler(async (req: Request, res: Response) => {
  const label = await labelService.createLabel(req.orgId!, req.params.id, req.user!.id, req.permissions!, req.body);
...
export const updateLabel = asyncHandler(async (req: Request, res: Response) => {
  const label = await labelService.updateLabel(req.orgId!, req.params.id, req.user!.id, req.permissions!, req.body);
...
export const deleteLabel = asyncHandler(async (req: Request, res: Response) => {
  const label = await labelService.deleteLabel(req.orgId!, req.params.id, req.user!.id, req.permissions!);
